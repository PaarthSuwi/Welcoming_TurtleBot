...
GREET_COOLDOWN_SECONDS = 15.0        # speak this often per guest (even if they stay in frame)

# Bigger ROI (centered); detection runs here first
ROI_REL = (0.20, 0.10, 0.80, 0.90)   # (x1, y1, x2, y2) in [0..1]

# Recognition settings
DET_MODEL = "hog"                    # or "cnn" if dlib is built with CUDA (best at distance)
CAM_INDEX = 0
...
DOWNSCALE = 0.5                      # 0.5 = half-size (more detail than 0.25)
UPSAMPLE = 1                         # small upsample helps farther faces
TARGET_CAM_W, TARGET_CAM_H = 1280, 720
MAX_BOX_AGE_FRAMES = 10

# Adaptive: if nothing found for N processed frames, do a full-frame sweep
...
# ==============================
# Dataset & Encodings (safe float cache)
# ==============================
...
class Recognizer(threading.Thread):
    """
    Detect/recognize on ROI every frame.
    If faces are missed briefly, do a full-frame sweep.
    """
    def __init__(self, cam: CameraGrabber, known_enc: np.ndarray, known_names: List[str]):
        ...
        self.box_age = 0
        self.missed = 0  # consecutive "no face found" counts

    def _prep_for_detection(self, bgr):
        # Gentle contrast boost to help in flat / dim lighting
        try:
    ...
    def run(self):
        while self.running:
            ...
            if roi.size > 0:
                ...
                # Map boxes back to full-frame coords
                inv = 1.0 / DOWNSCALE
                boxes = []
                for (t, r, b, l) in locs:
                    top = int(y1 + t * inv)
...
class AudioGreeter(threading.Thread):
    ...
    def greet(self, name: str):
        self.q.put(f"Welcome to SymbiTech Twenty Twenty Five, {name}!")
...
def _draw_hex_grid(img: Image.Image, spacing=48, color=(255,255,255,28)):
    ...
    a = spacing / 2
    b = spacing * np.sin(np.pi / 3)  # ~0.866
...
def make_welcome_banner(width: int, height: int, name: str,
    ...
    """Half-screen robotics banner: left avatar, right big text box (never clipped)."""
...
class KioskApp:
    def __init__(self, root: tk.Tk, dataset_dir: Path):
        ...
        self.banner_tk = None
        self.last_greet_time: Dict[str, float] = {}   # per-guest cooldown for TTS
        self.banner_ready = False

        # Re-render banner when size is real / changes
    ...
    def update_loop(self):
        ...
        if frame is not None:
            ...
            # --- Speak on a per-name cooldown (works even if they don't leave ROI) ---
            recognized_names = [n for n in names if n != "Unknown"]
            now_ts = time.time()
            for nm in recognized_names:
                last = self.last_greet_time.get(nm, 0.0)
                if (now_ts - last) >= GREET_COOLDOWN_SECONDS:
                    self.last_greet_time[nm] = now_ts
                    self.current_name = nm
                    self.showing_until = now_ts + WELCOME_SECONDS
                    self.render_welcome(nm)
                    self.audio.greet(nm)
                    break  # greet one person per cycle

            # Show camera on left (fills exactly half)
